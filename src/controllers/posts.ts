...
export const updatePost: RequestHandler<{pid: number}> = (req, res) => {
    const postId = req.params.pid
    const updatedPost = (req.body as {uid: number, timestamp: number, author: string, title: string, content: string, replies: Array<any> })
    ...
    POSTS[postIndex] = new Post(POSTS[postIndex].pid, updatedPost.uid, updatedPost.timestamp, updatedPost.author, updatedPost.title, updatedPost.content, updatedPost.replies)
    res.status(201).json({ message: 'Success!',  updatedPost: POSTS[postIndex]})
}

export const createPost: RequestHandler = (req, res) => {
    const postBody = (req.body as {uid: number, timestamp: number, author: string, title: string, content: string, replies: Array<any> })
    const newPost = new Post(Date.now(), postBody.uid, postBody.timestamp, postBody.author, postBody.title, postBody.content, postBody.replies)
